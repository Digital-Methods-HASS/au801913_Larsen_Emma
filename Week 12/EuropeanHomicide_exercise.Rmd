...
```{r plot-homicides-each-country, eval = TRUE}
#... YOUR CODE
Western_Europe %>% 
  ggplot(aes(x= Year, y= Homicide_rate, color=Countries))+
  geom_line()+
...
In the "Historical homicides in Western Europe" plot, the overall trend shows that homicide rates decrease over time.
In the "Danish Monarchs" plot, the trend indicates that the duration of kings reigns increases over time.
If we assume that shorter reigns were because of a higher risk of being killed, the two visualizations suggest the same pattern.
The homicide rates decline and the duration of kings’ reigns increases, because the homicide decrease over time.

```{r plot-danish_monarchs, eval = TRUE}
# YOUR CODE HERE:
read_csv2("data/Danish_monarchs-kopi.csv")
kings <- read_csv2("data/Danish_monarchs-kopi.csv")
kings

kings %>% 
  mutate(duration = Reign_end-Reign_start) %>% 
  mutate(mid_year= Reign_end + duration/2) %>% 
